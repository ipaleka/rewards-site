...
def added_allocations_for_addresses(request, addresses, txid):
    ...
    """
    if txid:
        messages.success(request, f"✅ Allocation successful TXID: {str(txid)}")
...
def claim_successful_for_address(request, address, txid):
    """Process successful claim and log the action.

    This function handles the successful clamation of an allocation by:
    - Displaying a success message with the address and transaction ID
    - Logging the clamation action in the user's profile

    Args:
        request: The HTTP request object containing user and session data
        address (str): Wallet address from which the allocation was claimed
        txid (str): Transaction ID of the successful clamation transaction

    Note:
        If txid is None or empty, the function returns without performing any actions.
    """
    if txid:
        messages.success(request, f"✅ Successfully claimed {address} (TXID: {txid})")
        Contribution.objects.user_has_claimed(address)
        request.user.profile.log_action("allocation_claimed", address)
