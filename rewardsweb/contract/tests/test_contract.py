...
def test_delete_application(context: AlgopyTestContext) -> None:
...
def test_delete_application_fails_for_non_admin(context: AlgopyTestContext) -> None:
    admin = context.any.account()
...
def test_setup(context: AlgopyTestContext) -> None:
...
def test_setup_fails_for_non_admin(context: AlgopyTestContext) -> None:
    admin = context.any.account()
    non_admin = context.any.account()
...
def test_setup_fails_if_already_set_up(context: AlgopyTestContext) -> None:
    admin = context.default_sender
...
def test_add_allocations(context: AlgopyTestContext) -> None:
    admin = context.default_sender
    ...
    assert raw in (None, b"", bytearray()), "Allocation box should be cleared or empty"

    raw = context.ledger.get_box(app.id, user2.bytes)
    assert raw in (None, b"", bytearray()), "Allocation box should be cleared or empty"


def test_add_allocations_updates_existing(
    context: AlgopyTestContext, monkeypatch
) -> None:
    admin = context.default_sender
...
def test_add_allocations_fails_for_non_admin(context: AlgopyTestContext) -> None:
    admin = context.any.account()
    non_admin = context.any.account()
...
def test_add_allocations_fails_for_different_length_arrays(
    ...
    with pytest.raises(AssertionError, match="Input arrays must have the same length"):
        with context.txn.create_group(active_txn_overrides={"sender": admin}):
            contract.add_allocations(
...
def test_claim(context: AlgopyTestContext, monkeypatch) -> None:
    admin = context.default_sender
...
def test_claim_fails_no_allocation(context: AlgopyTestContext) -> None:
    admin = context.default_sender
...
def test_claim_fails_period_ended(context: AlgopyTestContext) -> None:
    admin = context.default_sender
...
def test_claim_fails_not_opted_in(context: AlgopyTestContext) -> None:
    admin = context.default_sender
    ...
    with pytest.raises(AssertionError, match="Sender has not opted-in to the asset"):
        with context.txn.create_group(active_txn_overrides={"sender": user}):
            contract.claim()


def test_reclaim_allocation(context: AlgopyTestContext) -> None:
    admin = context.default_sender
...
def test_reclaim_allocation_fails_for_non_admin(context: AlgopyTestContext) -> None:
    admin = context.any.account()
    non_admin = context.any.account()
...
def test_reclaim_allocation_fails_no_allocation(context: AlgopyTestContext) -> None:
    admin = context.default_sender
...
def test_reclaim_allocation_fails_period_not_ended(context: AlgopyTestContext) -> None:
    admin = context.default_sender
